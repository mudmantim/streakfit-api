...
async function loadChallenges() {
    ...
    if (!result) return; // 401 handled in api() — token cleared, auth view shown
...
async function handleCheckIn(id, btn) {
    ...
    if (!result) return; // 401 handled — view already switched to auth

    if (result.status === 200) {
        await loadChallenges(); // reload all cards so streaks update correctly
    } else {
        btn.disabled    = false;
        btn.textContent = 'Check In';
        // "Already checked in today" arrives as 200, so this branch is
        // only reached on unexpected errors.
    }
}
...
async function init() {
    if (localStorage.getItem('streakfit_token')) {
        await loadChallenges();
        // If token was expired, api() cleared it and showed auth view already.
        // Only show dashboard if the token survived the check.
        if (localStorage.getItem('streakfit_token')) {
            showView('dashboard');
        }
    } else {
