...
def test_build_identity_satisfies_the_whole_contract(client):
    ...
    # Superset, not equality. The DEPLOYED build also publishes `atHead`,
    # which carries the head comparison so `state` can mean "could the
    # revision be read at all". Command requires these three; pinning the
    # set exactly would have deleted a field production already serves.
    assert {"latest", "appliedCount", "state"} <= set(body["migration"])
...
def test_an_unconfigured_coach_reports_unknown_not_healthy(client, monkeypatch):
    ...
    # FAIL, not UNKNOWN. This file and the deployed build's
    # tests/test_self_verification.py disagreed here, and the deployed
    # semantics were kept: we looked and established there is no key, so the
    # 503 is observed rather than unknown. The property this test exists for
    # -- that an unconfigured coach is never reported healthy -- holds either
    # way, and holds more strongly as FAIL.
    assert coach["status"] == "FAIL"
    assert coach["status"] != "PASS"
