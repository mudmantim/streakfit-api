...
- No account lockout / brute-force throttle beyond the 10/min IP limit — and that limit is `memory://` (per-worker, resets on deploy). Real brute-force protection needs shared rate-limit storage (roadmap).
