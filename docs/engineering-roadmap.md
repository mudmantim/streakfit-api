...
| # | Item | Benefit / Impact | Effort | Risk | Dependencies |
|---|---|---|---|---|---|
| ~~M1a~~ | ~~**Rate limits keyed on a proxy address**~~ — **RESOLVED 2026-07-26** | ~~Limits were enforced per Render-internal address, so the effective ceiling was ~6-7× configured (`/api/register` admitted 30 of 50 and 37 of 60 probes against a 5/min limit)~~ — fixed by `ProxyFix(x_for=2)`; regression-gated by `post_deploy_check.py` asserting exactly 5/min. See [operations/rate-limiting-client-ip.md](operations/rate-limiting-client-ip.md) | — | — | — |
| M1b | **Rate-limit storage is `memory://`** — process-local, resets every deploy | Medium — the key is now correct, so limits work; what remains is that they reset on each deploy (the coach's `10 per day` silently restarts) and would partition if a second worker/instance is added. **Do not do this before M1a** — shared storage with a proxy-keyed limiter would have collapsed all users onto one bucket | M | Medium (new infra dependency) | Render Key Value, **free plan** (25 MB, no persistence) — decided 2026-09-21; free is shared across workers, which is the half that matters, and `memory://` already loses counters on every deploy. **Outage behaviour measured 2026-09-21 before adopting** — stopping the backend used to 500 every throttled route for 15s, including `/api/health`; fixed and gated, see [operations/rate-limit-backend-outage.md](operations/rate-limit-backend-outage.md) |
| M9 | **Team-owning QA smoke accounts accumulate and cannot be cleared automatically** — 12 remain in production as of 2026-07-26 | Low operationally (they are inert `qa_smoke_*` rows owning `Smoke Test <tag>` teams), but the count grows by one per `verify_all` run against production, so it is a slow leak in the smoke process rather than a one-off. **Blocked on a policy decision, not on effort:** `cleanup_qa_smoke.py` deliberately refuses team owners, and `delete_user_account` only proceeds with an explicit `allow_team_owner=True`, which would orphan the team. Direct SQL and that flag are both **ruled out** until a reviewed approach exists (Tim, 2026-07-26). The durable fix is upstream — have the verification suite tear down the team it creates, or add a reviewed archive/delete path for team-owning smoke data | S (teardown) / M (archive path) | Medium — touches shared team data and the deletion policy in [adrs/0007](adrs/0007-qa-cleanup-safe-vs-blocked.md) | A decided policy first; then either suite teardown or a `--allow-team-owner` path with its own tests |
...
4. **Rate limiter tripped at 10/min during profiling** — confirms the limit is active and low. This observation originally concluded "real fix is shared storage (M1), not raising limits", which was **wrong about the priority**: the binding defect was the *key*, not the storage. Until 2026-07-26 the limiter keyed on a Render-internal address, so a distributed attacker was not merely poorly contained — every limit was ~6-7× looser than configured. Fixed by `ProxyFix(x_for=2)` (M1a). Shared storage (M1b) is still worthwhile but is now the smaller half. Shared-IP/NAT clients sharing a bucket is inherent to per-IP limiting.
