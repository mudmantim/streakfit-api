...
| 15 | **Rate limiting / brute force** | 🔵 | Partial — login 10/min, register 5/min, coach 10/day+3/min. Gaps: `memory://` storage (per-worker, resets on deploy) and no `ProxyFix`/XFF handling behind Render's proxy. See **Workstream 7** (rate-limit correctness) — documented, interim strategy recommended, no new paid service. | — |
