...
| `STREAKFIT_NOTIFY_CHANNEL` | No | — | unset | Which delivery channel moderation notices use. **Unset → nothing is delivered and nothing is claimed as delivered** — the honest default until a provider is chosen. `console` prints but deliberately certifies nothing, and is reported as **not** configured by `moderation.delivery_configured` for exactly that reason. |
| `RESEND_API_KEY` | With Resend: **Yes** | 🔒 | unset | Resend API credential. Required together with `STREAKFIT_NOTIFY_FROM` and `STREAKFIT_NOTIFY_TO` — with any one missing the channel refuses to build and `moderation.delivery_configured` FAILS naming the missing variable. Never logged, never in a payload, never in an error. |
| `STREAKFIT_NOTIFY_FROM` | With Resend: **Yes** | — | unset | Verified sender address. Resend requires a verified domain; a subdomain (`alerts.streakfit.pro`) is recommended so sending reputation stays off the root domain. |
| `STREAKFIT_NOTIFY_TO` | With Resend: **Yes** | — | unset | The **one** address alerts go to. The recipient is configuration and never data: `send()` has no recipient parameter, so a bug in notice generation can send the wrong sentence but not to the wrong person. |
| `STREAKFIT_RETENTION_SETTLE_S` | No | — | `20` | Seconds between the in-process worker starting and its first pass. The worker works then sleeps, so this is the whole start-up delay — not an hour. Monitoring reports UNKNOWN rather than FAIL inside this window plus a two-minute grace. |
