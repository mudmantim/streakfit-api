...
| 4b | **The verification branch exists only on this machine** | Owner | `streakfit-verification-support` (`5a9b8f9`) in `~/projects/mudman-command` is committed but **not on origin**, and pushing is outside my authorisation. A `git bundle` copy sits at `~/mudman-command-backups/streakfit-verification-support-5a9b8f9.bundle` (verified complete, on `/home`, survives a reboot) as partial insurance — it survives a branch deletion or a bad checkout, but not a disk loss. Deliberately **outside** the working copy: a peer review pointed out that a backup inside it would be swept into the repository by anybody's `git add -A`. |
